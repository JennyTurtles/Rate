...
public class DoctorService {
    ...
    public Msg checkGraduateStudent(MultipartFile file,int index, Integer groupid) {
...
//                    }
                ...
                if (isTypeEmpty) {
                    String tips = "第【" + (j + 1) + "】行的学生类别存在问题，请确认";
                    msg.setCode(500);
                    msg.setMsg("第【" + (j + 1) +"】行的学生类别存在问题，请确认");
                    return msg;
                }
                if (isTuturNoEmpty) {
...
//                    return msg;
    ...
    public RespBean updatePoint1(Integer studentID) {
        try {
            int res = doctorMapper.updatePoint1(studentID);
            if (res > 0) {
