  ...
  <div>
    ...
    >
      <div>
        ...
        <label style="margin-left:16px">奖励名称：</label>
        <input type="text"
               style="margin-left:5px;width:80px;height:30px;padding:0 30px 0 15px;
                border:1px solid lightgrey;color:lightgrey;
                border-radius:4px;color:grey"
        ...
        <label style="margin-left:40px;">奖励状态：</label>
        <el-select
    ...
    <el-dialog
      ...
      >
        ...
        <el-form-item label="奖励级别:">
          <span>{{ emp.awardLevel }}</span
          ><br />
        </el-form-item>
        <el-form-item label="奖励类别:">
          <span>{{ emp.awardType.name }}</span
          ><br />
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    rolePass(state) {
      let url = "/award/basic/edit_state?state=" + state + "&ID=" + this.emp.id;
    ...
    async doAddOper(state,remark,awardID) {
      ...
      await this.searchEmps(this.currentPage, this.pageSize);
    },
    rowClass(){
