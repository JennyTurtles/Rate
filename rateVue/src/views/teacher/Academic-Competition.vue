...
export default {
  ...
  methods: {
    ...
    changePaperPoint(data, point) {
      ...
      return this.postRequest(`/competition/basic/editPoint/${data.id}`, params).then()
    },
    ...
    auditing_commit(state){
      this.loading = true;
      if(this.role == 'admin' && (state.indexOf('pass') >= 0 || state.indexOf('reject') >= 0) && this.currentCompetition.state == 'commit') { //管理员通过 有提示
