...
<!--        </div>-->
    ...
    <el-dialog
      ...
      <span slot="footer" class="dialog-footer" :model="currentCompetition">
            <el-button
                id="but_pass"
                v-show="((currentCompetition.state=='commit' && role == 'teacher') || (currentCompetition.state=='tea_pass' && role == 'admin')) ? true:false"
                @click="(()=>{
                  if (role == 'teacher')
...
export default {
  ...
  methods: {
    ...
    auditing_commit(num){
      this.loading = true;
      let url = "/competition/basic/edit_state?state=" + num + "&ID=" + this.currentCompetition.id;
      this.dialogVisible_show=false
      if(num.indexOf('reject') >= 0){
        this.currentCompetition.remark = this.reason;
      }
      this.getRequest(url).then((resp) => {
        this.loading = false;
        if (resp) {
          this.currentCompetition.state = num
          this.$message({
            type: 'success',
            message: '操作成功'
          })
          this.doAddOper(num, this.reason, this.currentCompetition.id);
        }
      })
    },
