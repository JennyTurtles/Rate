...
<script>
export default {
  name: "DoctorManageAchievementInfo",
  data() {
    return {
      isImage: false,
      isPdf: false,
  ...
  methods: {
    ...
    changeStudentPoint(data, point) {
      ...
      if(point == 0) { //减法
        return this.postRequest('/doctorM/basic/updateScoreSub', params).then()
      } else { //加法
        return this.postRequest('/doctorM/basic/updateScore', params).then()
      }
    },
    //点击对话框中的确定按钮 触发事件
