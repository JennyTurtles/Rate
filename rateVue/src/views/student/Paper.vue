  ...
  <div>
      ...
      >
        ...
        >
          ...
          <div style="margin-left: 20px; font-size: 10px;">
            ...
            &nbsp;&nbsp;&nbsp;&nbsp;
            <span style="color: #409eff; cursor: pointer" @click="openCheckVue">
              查看期刊审核进度
            </span>
          </div>
        </el-form-item>
    ...
    >
      ...
      >
        ...
        <div
          v-show="emp.url == '' || emp.url == null ? false : true"
          style="margin-left: 80px"
        >
          <div>
            <el-button @click="previewMethod('1')" v-show="isImage || isPdf"
              >预览</el-button
            >
        ...
        <div>
          ...
          >
            ...
            >
              ...
              <div v-show="item.remark == '' ? false : true">
                驳回理由：{{ item.remark }}
              </div>
            </div>
          </div>
    ...
    >
      <template v-if="isPdf">
        <vue-office-pdf :src="previewUrl" style="height: 100vh" />
      </template>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    handleChangeFilesPublication(file) {
      //文件列表数量改变
    ...
    queryPublication() {
      ...
      this.timer = setTimeout(() => {
        if (this.publish.publicationName) {
          var url = `/publication/getInf/${encodeURIComponent(
            this.publish.publicationName
          )}`;
          this.getRequest(url).then((resp) => {
            if (resp && resp.obj) {
              ...
              this.publishToDatabase.check_duplicates.indicatorId =
                indicatorIds;
        ...
        this.$refs["publicationForm"].validate(async (valid) => {
          if (valid) {
            ...
            for (let i = 0; i < length; i++) {
              if (
                year == duplicates.year[i] &&
                indicatorId == duplicates.indicatorId[i]
              ) {
    ...
    debounceSearchInput(val) {
      //第一步得到期刊全称 第二步 名称+year 获取积分
      if (!val) {
        return;
      }
      if (!this.isInitEditDialog && this.currentEmp.id) return;
      this.loadingPublicationSearch = true;
      let url = `/publication/basic/listByNameYear/${encodeURIComponent(
        val
        ...
        if (resp) {
          if (resp.obj) {
            ...
            this.publication_detail =
              ...
              year +
              "年，按照东华大学毕业要求，属于" +
              resp.obj.indicatorList[0].order +
              "类";
    ...
    showEditEmpView(data) {
      ...
      this.currentEmp.startPage = this.$set(
        this.currentEmp,
        "startPage",
        this.currentEmp.pubPage.split("-")[0]
      );
      this.currentEmp.endPage = this.$set(
        this.currentEmp,
        "endPage",
        this.currentEmp.pubPage.split("-")[1]
      );
    ...
    deleteEmp(data) {
      ...
      this.$confirm("此操作将永久删除【" + data.name + "】, 是否继续?").then(
        () => {
          Promise.all([
            this.deleteEmpMethod(data),
            this.deleteOperationList(data),
          ])
            .then((res) => {
              this.$message.success("删除成功!");
              this.initEmps();
    ...
    deleteOperationList(data) {
      ...
      return new Promise((resolve, reject) => {
        this.postRequest("/oper/basic/deleteOperationList", params).then(
          (res) => {
            resolve("success");
          }
        );
    ...
    showInfo(data) {
      ...
      this.getRequest(
        "/oper/basic/List?prodId=" + data.id + "&type=学术论文"
      ).then((resp) => {
        this.loading = false;
        if (resp) {
