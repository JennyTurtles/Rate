  ...
  <div>
    <div>
      ...
      >
        <div>
          <el-input
            placeholder="请输入学术专著和教材名称进行搜索，可以直接回车搜索..."
    ...
    <el-dialog :title="title" :visible.sync="dialogVisible" width="30%" center>
      ...
      >
        <el-form-item label="学术专著和教材名称:" prop="name">
          <el-input
            size="mini"
            style="width: 200px"
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        name: [{ required: true, message: "请输入学术专著和教材名", trigger: "blur" }],
        startDate: [
          { required: true, message: "请输入学术专著和教材时间", trigger: "blur" },
        ],
        scoreItemCount: [
          {
            required: true,
  ...
  methods: {
    ...
    emptyEmp() {
      this.emp = {
        ...
        name: "",
        scoreItemCount: "0",
        comment: "学术专著和教材备注example：关于xxx的学术专著和教材",
      };
    },
      ...
      if (this.emp.id) {
        ...
        this.$refs["empForm"].validate((valid) => {
          if (valid) {
            ...
            const _this = this;
            this.postRequest("/center/basic/insert", _this.emp).then(
              (resp) => {
                if (resp) {
                  this.dialogVisible = false;
                  this.initEmps();
