...
<!--     </div>-->
   ...
   >
    ...
    <el-table-column align="center" width="650" label="操 作">
     <template slot-scope="scope">
      ...
      >
      <el-button
          @click="showSubActivityGroup(scope.row)"
          style="padding: 4px"
          size="mini"
          icon="el-icon-plus"
          type="primary"
  ...
  <el-dialog :title="title" :visible.sync="dialogVisible" width="46%"
             @close="emp_edit={};haveComment = false;haveSub = false;gradeFormType = 0;radio = 1;currentClone='';currentCloneid=''"
             center>
   ...
   >
    ...
    <div>注：克隆活动时会克隆对应的评分项、信息项、展示项、子活动的对应信息</div>
    <br/>
   </el-form>
  ...
  <el-dialog :visible.sync="dialogActivityPermission" width="70%" center @close="closeDialogOfAddPermission">
   ...
   <el-table @selection-change="handleSelectionChange" :data="currentInstitutionAdminList" ref="addPermissionTable"
    ...
    <el-table-column type="selection" width="35px" :reserve-selection="true"
                     :selectable="permissionSelectable"></el-table-column>
...
export default {
 ...
 methods: {
  ...
  goExportGradeForm() {
   ...
   if (this.user.role !== 3) // 如果为秘书，下载时不会保存
