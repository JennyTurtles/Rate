    ...
    <div v-show="!resetPassShow">
      <el-form class="resetPassContainer" :label-width="labelWidth" >
        <el-form-item label="请输入用户名:">
          <el-input style="width: 60%"  v-model="username" @input="checkusername"></el-input>
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    //输入身份证号进行查找判断
    checkusername() {
      let url = '/registerUser/getUserByIdNumber?role=' + this.role + '&username=' + this.username
    ...
    reset() {
      if (this.newPass === this.confirmNewPass && this.resetPassShow) {
        const encryptedPassword = ExpertService.sh1(this.newPass); // 使用与注册相同的加密方法
        this.$set(this.user, "password", encryptedPassword); // 替换明文密码为加密后的值
